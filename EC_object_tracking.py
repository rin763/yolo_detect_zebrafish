...
class ObjectTracker:
    ...
    def collect_training_data(self, video_path, save_dir):
        ...
        while cap.isOpened():
            ...
            if results[0].boxes is not None and len(results[0].boxes) > 0:
                self.update_tracking(frame_idx, results[0].boxes)
            frame_idx += 1
        ...
        print(f"[DEBUG] Track history object count: {len(self.track_history)}")
        for obj_id, trajectory in self.track_history.items():
            print(f"Object {obj_id} has {len(trajectory)} frames")

            if len(trajectory) >= self.sequence_length + 1:
                traj_array = np.array(trajectory)
                for i in range(len(traj_array) - self.sequence_length):
                    clip = traj_array[i:i+self.sequence_length+1]
                    filename = f"track_{obj_id}_{track_count}.npy"
                    filepath = os.path.join(save_dir, filename)
                    np.save(filepath, clip)
                    track_count += 1

        print(f"[INFO] Saved {track_count} track sequences to {save_dir}")

if __name__ == "__main__":
    # モデルのパスを指定
    model_path = r"C:\Users\et439\OneDrive\桌面\project\Rin\train_results\weights\best.pt"  # あなたのYOLOモデルのパスに変更してください
    
    # トラッカーの初期化
    tracker = ObjectTracker(model_path)
    
    # 動画のパスを指定
    video_path = r"C:\Users\et439\OneDrive\桌面\project\Rin\video\processed_train_video_left.mp4"  # 処理したい動画のパスに変更してください
